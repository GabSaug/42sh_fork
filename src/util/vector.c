...
# 5 "src/util/vector.c" 2

# 1 "src/util/vector.h" 1
# 7 "src/util/vector.c" 2
...
struct vector* v_create(void)
{
  ...
  new_vector->data = my_malloc(16 * sizeof (void*));
  return new_vector;
}
...
void* v_get(struct vector* v, size_t i)
{
  if (i >= v->size)
    return 
# 65 "src/util/vector.c" 3 4
          ((void *)0)
# 65 "src/util/vector.c"
              ;
  return v->data[i];
}
